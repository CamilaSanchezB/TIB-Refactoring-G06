...
function handleGet($conn)
{
    ...
    if (isset($input['student_id'])) {
        $studentsSubjects = getSubjectsByStudent($conn, $input['student_id']);
...
function handlePost($conn)
{
    ...
    $row = array_filter($studentsSubjects, function ($subject) use ($input) {
        return $subject['subject_id'] == $input['subject_id'];
    });

    if (count($row) > 0) {
        echo json_encode(["error" => "El alumno ya esta asignado a la meteria"]);
